...
@use "./node_modules/@theholocron/config/src" as config;

// override parts of the config with a different set of breakpoints
// still includes all others untouched
@use "./node_modules/@theholocron/config/src" as config with (
    $breakpoints: (
        "xs": "0",
        "sm": "480px",
        "md": "768px",
...
@media (min-width: map.get(config.$breakpoints, "xs")) {
   ...
}
```
...
The best way to find out what's available is to check out the [source code](./src/_index.scss) or [tests](./tests/_config.scss) but you can also take a look at [the-holocron.github.io/astromech/config](http://the-holocron.github.io/astromech/config/) or the [README](./src/README.md) which have been generated by [SassDoc](http://sassdoc.com) and [`sassdoc-to-markdown`](https://github.com/hidoo/unit-sass/blob/master/packages/sassdoc-to-markdown).
